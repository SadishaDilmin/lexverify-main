...
-- Add audit columns to ai_reports for tracking modifications
...
-- Create trigger function to log ai_report modifications to audit_log
CREATE OR REPLACE FUNCTION public.log_ai_report_modification()
RETURNS TRIGGER AS $$
DECLARE
  v_case_ref TEXT;
BEGIN
  -- Update modification tracking columns
  NEW.modified_at = now();
